...
// DISABLE ONLY WOOCOMMERCE HANDLERS, KEEP CUSTOM
add_action('wp_loaded', function() {
    // Remove WooCommerce's form handler
    remove_action('wp_loaded', array('WC_Form_Handler', 'add_to_cart_action'), 20);
    
    // Remove WooCommerce AJAX handlers
...
// CLEAN SINGLE ADD TO CART HANDLER
function ls_add_to_cart_handler() {
    ...
    // Add exactly 1 item with normal stacking
    if ($variation_id) {
        $cart_item_key = WC()->cart->add_to_cart($product_id, 1, $variation_id, $cleaned_variation);
    } else {
...
add_action('wp_ajax_ls_add_to_cart', 'ls_add_to_cart_handler');
add_action('wp_ajax_nopriv_ls_add_to_cart', 'ls_add_to_cart_handler');
...
/**
 * FORCE QUANTITY = 1 FOR ALL CART ADDITIONS
 */
add_filter('woocommerce_add_to_cart_quantity', function($quantity, $product_id) {
    return 1;
}, 10, 2);

// Override WooCommerce's quantity handling for variations
    ...
    <div class="custom-cart-wrapper" style="position: absolute; top: 50%; right: 20px; transform: translateY(-50%); z-index: 999;">
        <style>
            @media (min-width: 481px) and (max-width: 1024px) {
                .custom-cart-wrapper { right: 20px !important; left: auto !important; top: 50% !important; transform: translateY(-50%) !important; }
            }
        </style>
...
// UPDATED get_custom_cart_data function with enhanced sale pricing support
function get_custom_cart_data() {
    ...
    if (WC()->cart) {
        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
            ...
            // Get pricing information - ENHANCED FOR SALE DISPLAY
            ...
            $cart_items[] = array(
                'key' => $cart_item_key,
                'name' => $product->get_name(),
                'price' => wc_price($current_price), // Formatted current price
                'unit_price' => $current_price, // Raw unit price for calculations
                'regular_price' => $regular_price, // Raw regular price
                'sale_price' => $sale_price, // Raw sale price
                'on_sale' => $is_on_sale, // Sale status
                'image' => $image_url,
                'variation' => $variation_text,
                'quantity' => $quantity,
...
// CUSTOM ADD TO CART - FORCE QUANTITY 1
function londonsneaks_custom_add_to_cart() {
    error_log('LONDONSNEAKS AJAX Handler Called: ' . print_r($_POST, true));
    ...
    error_log('Adding to cart: Product ' . $product_id . ', Variation ' . $variation_id);
    ...
    // FORCE quantity to 1 by overriding any POST data
    $_POST['quantity'] = 1;
    $_REQUEST['quantity'] = 1;
    
    // Count cart before
    $cart_count_before = WC()->cart->get_cart_contents_count();
    error_log('Cart count before: ' . $cart_count_before);
    
    // Add exactly 1 item with explicit quantity
    if ($variation_id) {
        $cart_item_key = WC()->cart->add_to_cart($product_id, 1, $variation_id, $cleaned_variation);
    } else {
        $cart_item_key = WC()->cart->add_to_cart($product_id, 1);
    ...
    error_log('Cart count after: ' . $cart_count_after);
    error_log('Items added: ' . ($cart_count_after - $cart_count_before));
...
add_action('wp_ajax_londonsneaks_add_to_cart', 'londonsneaks_custom_add_to_cart');
add_action('wp_ajax_nopriv_londonsneaks_add_to_cart', 'londonsneaks_custom_add_to_cart');

// TEST HANDLER - TEMPORARY
add_action('wp_ajax_test_handler', 'test_ajax_handler');
add_action('wp_ajax_nopriv_test_handler', 'test_ajax_handler');
...
add_action('wp_head', function() {
    ...
    <script>
    // Disable WooCommerce's default add to cart
    jQuery(document).ready(function($) {
        // Remove WooCommerce's default handlers
        $('body').off('click', '.single_add_to_cart_button');
...
// Completely disable WooCommerce's default add to cart AJAX
add_action('init', function() {
    // Remove WooCommerce's AJAX add to cart action
    remove_action('wp_ajax_woocommerce_add_to_cart', array('WC_AJAX', 'add_to_cart'));
    remove_action('wp_ajax_nopriv_woocommerce_add_to_cart', array('WC_AJAX', 'add_to_cart'));
    
    // Remove variation handling
    remove_action('wp_ajax_woocommerce_add_to_cart_variable_product', array('WC_AJAX', 'add_to_cart_variable_product'));
    remove_action('wp_ajax_nopriv_woocommerce_add_to_cart_variable_product', array('WC_AJAX', 'add_to_cart_variable_product'));
});
...
// Hook the custom checkout handler to WordPress AJAX
add_action('wp_ajax_process_custom_checkout', 'handle_custom_checkout_process');
add_action('wp_ajax_nopriv_process_custom_checkout', 'handle_custom_checkout_process');

function handle_custom_checkout_process() {
    // Verify nonce
    ...
    try {
        ...
        if (!isset($available_gateways[$payment_method])) {
            wp_send_json_error('Invalid payment method selected');
        }
        ...
        // Prepare checkout data in WooCommerce format
        $checkout_data = array(
            ...
            'billing_country' => sanitize_text_field($_POST['billing_country'] ?? 'GB'),
            ...
            'shipping_country' => sanitize_text_field($_POST['shipping_country'] ?? 'GB'),
            'payment_method' => $payment_method,
            'terms' => 1,
            'createaccount' => 0
        );
        
        // If billing address is not different, copy shipping to billing
        if (empty($_POST['different_billing'])) {
            $checkout_data['billing_first_name'] = $checkout_data['shipping_first_name'];
            $checkout_data['billing_last_name'] = $checkout_data['shipping_last_name'];
            $checkout_data['billing_address_1'] = $checkout_data['shipping_address_1'];
            $checkout_data['billing_city'] = $checkout_data['shipping_city'];
            $checkout_data['billing_postcode'] = $checkout_data['shipping_postcode'];
            $checkout_data['billing_country'] = $checkout_data['shipping_country'];
        }

        // Validate required fields
        $required_fields = array('billing_first_name', 'billing_last_name', 'billing_address_1', 
                               'billing_city', 'billing_postcode', 'billing_email', 'shipping_first_name',
                               'shipping_last_name', 'shipping_address_1', 'shipping_city', 'shipping_postcode');
        ...
        // Set the posted data for WooCommerce
        foreach ($checkout_data as $key => $value) {
            $_POST[$key] = $value;
        }
        
        // Create order using WooCommerce's proper method
        ...
        if (is_wp_error($order_id)) {
            wp_send_json_error('Order creation failed: ' . $order_id->get_error_message());
        }
        
        $order = wc_get_order($order_id);
        
        if (!$order) {
            wp_send_json_error('Failed to retrieve order');
        }
        ...
        // Set WooCommerce session order ID for payment gateway
        WC()->session->set('order_awaiting_payment', $order_id);
        
        // Clear cart after order creation but before payment processing
        WC()->cart->empty_cart();
        
        // Special handling for Amazon Pay and redirect-based gateways
        if (strpos($payment_method, 'amazon') !== false || 
            strpos($payment_method, 'paypal') !== false ||
            in_array($payment_method, array('amazon_payments_advanced', 'woocommerce_amazon_payments_advanced'))) {
            
            // For Amazon Pay and other redirect gateways, return the payment URL
            ...
            // Process payment through the gateway for other methods
...
// Ensure WooCommerce checkout scripts are loaded on custom checkout page
function enqueue_checkout_scripts() {
    if (is_page_template('page-custom-checkout.php') || is_page('custom-checkout')) {
...
// Enhanced cart data function with sale pricing
function get_custom_cart_data_enhanced() {
    if (!WC()->cart) {
        return array(
            'cart_count' => 0,
            'cart_items' => array(),
    ...
    // Process cart items with enhanced pricing
    foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
        $product = $cart_item['data'];
        $product_id = $cart_item['product_id'];
        $variation_id = $cart_item['variation_id'];
        ...
        $cart_items[] = array(
            ...
            'unit_price' => $is_on_sale ? $sale_price : $regular_price,
            'on_sale' => $is_on_sale,
...
// Update the existing AJAX handler
add_action('wp_ajax_get_custom_cart_data', 'handle_get_custom_cart_data_enhanced');
add_action('wp_ajax_nopriv_get_custom_cart_data', 'handle_get_custom_cart_data_enhanced');

function handle_get_custom_cart_data_enhanced() {
    check_ajax_referer('custom_cart_nonce', 'nonce');
    
    $cart_data = get_custom_cart_data_enhanced();
    
    wp_send_json_success($cart_data);
}

// Enqueue custom cart script with localStorage support
function enqueue_custom_cart_scripts() {
    wp_enqueue_script('custom-cart-js', get_template_directory_uri() . '/js/custom-cart.js', array('jquery'), '1.0.0', true);
    wp_localize_script('custom-cart-js', 'custom_cart_ajax', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('custom_cart_nonce'),
        'home_url' => home_url('/')
...
// Add JavaScript to handle cart auto-open on homepage
function add_cart_autoopen_script() {
    if (is_front_page() || is_home()) {
        ?>
        <script>
        jQuery(document).ready(function($) {
            // Enhanced auto-open check with multiple attempts
            function checkForAutoOpen() {
                if (localStorage.getItem('openCartOnLoad') === 'true') {
                    localStorage.removeItem('openCartOnLoad');
                    console.log('Auto-opening cart from localStorage flag');
                    
                    // Wait for cart to be fully loaded
                    setTimeout(function() {
                        if (typeof window.openCustomCart === 'function') {
                            window.openCustomCart();
                        } else {
                            console.log('Cart function not ready, trying again...');
                            setTimeout(function() {
                                if (typeof window.openCustomCart === 'function') {
                                    window.openCustomCart();
                                }
            ...
            checkForAutoOpen();
            setTimeout(checkForAutoOpen, 1000);
        });
        </script>
        <?php
    }
}
add_action('wp_footer', 'add_cart_autoopen_script');
?>